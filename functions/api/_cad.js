...
export async function lookupCad(env, codeRaw, lat, lng) {
  ...
  /* --- დონე 1: მისამართი ---
     ⚠️ 2026-09-01, George-ის მოთხოვნით — root cause: ქვემოთ placeMatches()
     ითხოვს რეალურ lat/lng-ს (პინის მდებარეობას), მაგრამ თუ ნიშნული
     საერთოდ არ დასმულა (ეს ხდება ხშირად — კოდი ხშირად გეომეტრიის
     გარეშეა, ტექსტური მისამართით, ამიტომ ზემოთ დონე-2-ის ავტო-განთავსება
     ვერ ხდება, იხ. ზემოთ კომენტარი), url.searchParams.get('lat') აბრუნებს
     null-ს — `+null` კი არა NaN-ს, არამედ 0-ს! ანუ placeMatches() რეალურ
     მისამართს (0,0)-სთან ("null island") ადარებდა და ყოველთვის რამდენიმე
     ათასი კილომეტრის აბსურდულ "პინი X კმ-ითაა დაშორებული" შეცდომას
     აბრუნებდა — მაშინაც კი, როცა ნიშნული საერთოდ არ არსებობდა (და
     კოდიც, და მისამართიც, სავსებით სწორი იყო). იგივე geoOk() შემოწმება,
     რასაც ზემოთ დონე-2 იყენებს (Number.isFinite + საქართველოს
     საზღვრები), აქაც უნდა გვქონდეს — თუ ნამდვილი პინი არ არის,
     მშვიდად ვამბობთ, რომ ჯერ არ დასმულა, ნაცვლად ყალბი დაშორების
     გამოთვლისა. */
  if (!geoOk(lat, lng)) {
    out.cad_ok = 0;
    out.why = 'კოდი და მისამართი ნაპოვნია რეესტრში — ნიშნული ჯერ არ დასმულა რუკაზე. დასვი ნიშნული, ან მოდერატორი ხელით გადაამოწმებს.';
    return out;
  }
